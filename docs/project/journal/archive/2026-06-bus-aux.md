...
**À faire (doc) :** ~~journal à 6 sessions visibles (S26→S31) = plafond atteint ; archiver le thread liaison-monture par milestone~~ **fait (2026-06-29)** : S26→S30 archivées dans ce fichier (`journal/archive/2026-06-bus-aux.md`) (S31→S33 visibles).
...
- **Page de câblage autonome créée** : `aux-rx-buffer-4093.html` (schéma DIP14 + netlist + chiffres de validation + procédure + jour/nuit) — page supprimée le 2026-06-29 (voir note en tête).
...
**Montage construit — driver open-collector 2× NPN** : Q1 inverseur + Q2 driver, 4× **4,7 kΩ** (Rb1/Rc/Rb2/Rpu). Page câblage autonome créée : `aux-single-wire-cablage.html` (schéma SVG + netlist + brochage + toggle jour/nuit) — page supprimée le 2026-06-29 (voir note en tête). Test logique sur table OK (TX→GND → DATA bascule).
