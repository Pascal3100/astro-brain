...
2-bis. **Cause racine probable des symptômes D-pad (n°2) : tracking sidéral actif sur un modèle poubelle.** Constaté en live à 21:50 : `tracking: sidereal` + `is_aligned: true` alors que l'alignement était bidon — chaque `record()` du wizard pousse un `sync_radec` à la monture **et arme le suivi** (`services/alignment.py:147`). Après chaque relâchement du D-pad, le driver réengage le suivi (~1,3 s), qui « poursuit » une cible calculée sur le modèle faux → mouvements parasites (« compensations »), jog noyé à basse vitesse, impression de non-arrêt à haute vitesse. **À re-tester sur état propre avant de traiter le n°2 comme un bug distinct.** Question de fond à trancher : armer le suivi dès la 1ʳᵉ étoile est fidèle à la raquette, mais un suivi assis sur des syncs erronés est pire que pas de suivi — et il n'existe plus aucune commande pour l'éteindre depuis l'app (« le suivi se constate, il ne se commande plus »).
...
4. **✅ Corrigé à chaud — reprise silencieuse de session + modèle persisté irrévocable + swap en erreur + CENTRÉ sous la barre Android.** Quatre défauts liés, constatés d'un coup : « Commencer » reprenait la session backend existante avec ses records poubelle (finalisation après une seule étoile « centrée »), `cancel()` ne révoquait pas le modèle SQLite (rehydrate le restaurait au boot), le swap re-dispatchait `WizardStarted`, et l'écran par-étoile débordait. Fixes : start-à-neuf (cancel best-effort avant start), `cancel()` appelle `alignment_repo.clear()`, swap via `StarSwapRequested(idx, star)` dans le bloc, écran d'erreur avec bouton RECOMMENCER, colonne scrollable. Commits `73c48d0` + `285cd7b`. **Reste ouvert de la même famille** : rien ne purge les points de sync côté driver entre deux sessions wizard (le power-cycle monture s'en est chargé ce soir — à vérifier : le driver les garde-t-il en RAM seulement ?).
