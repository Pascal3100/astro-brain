...
- **ARP Pi → pont ESP32 : entrée statique à rendre durable (S53, défaut #5 root-causé).** ⚠️ **Phénomène distinct du précédent, ne pas les confondre.** Ici c'est le chemin **Pi → 192.168.1.200** qui casse : la résolution ARP reste `INCOMPLETE` côté Pi (`No route to host` sur `ping` comme sur `nc`) alors que la workstation joint le pont au même instant, et que le reste du réseau du Pi est sain. Le power-save du Pi (item ci-dessus) explique un Pi injoignable *depuis l'extérieur*, pas ça. Contournement qui marche à 100 % : `sudo ip neigh replace 192.168.1.200 lladdr 30:ae:a4:40:a8:38 dev wlan0 nud permanent` → ping 0 % de perte, TCP 2000 ouvert. **Mais l'entrée est en RAM et disparaît au reboot** (constaté en séance). À rendre durable : unit systemd oneshot `After=network-online.target`, ou hook dispatcher NetworkManager sur `up`. Côté fautif non tranché — broadcast ARP du Pi qui ne sort pas, ou pont qui n'y répond pas ; test discriminant : `arping` unicast vers la MAC connue. Si c'est le pont, le vrai correctif est firmware (répondre à l'ARP) et l'entrée statique devient inutile — donc **faire le test avant d'industrialiser le contournement**.
