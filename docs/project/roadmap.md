...
- 🌫 **Cordwrap (anti-enroulement câble) — armé par la raquette, jamais par nous** : le sniff du bus AUX du 2026-08-27 montre que la raquette envoie `MC_ENABLE_CORDWRAP` (0x38) puis `MC_SET_CORDWRAP_POS 0` (0x3a) sur l'axe AZM **juste après avoir validé l'alignement**, dans la même rafale que l'armement du suivi (journal S57). Notre backend ne touche à rien : la protection est donc absente de nos sessions. Pas de fork driver nécessaire, contrairement au backlash, et c'est précisément le cas d'application de l'[ADR 2026-08-27](decisions.md) « s'appuyer sur ce qu'expose le driver » — `indi_celestron_aux` expose déjà `CORDWRAP` (on/off), `CORDWRAP_POS` (8 azimuts) et `CW_BASE` (encodeurs / positions d'alignement, défaut encodeurs) dans l'onglet Cord Wrap (`celestronaux.cpp:337/348/353`). Reste à décider *quand* l'armer côté backend (à la 1ʳᵉ étoile, comme le suivi ?) et si la position s'expose côté app ou reste un réglage implicite.
