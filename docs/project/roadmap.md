...
- **Oracle / Éphémères** : plan de données de référence **autonome du Pi** — module `oracle/` généré par GitHub Actions (skyfield + MPC) → `reference.sqlite`, consommé par l'app et le Pi (cache local, hors ligne). ✅ **Tranche 1 producteur livrée 2026-07-24** (module `oracle/`, comètes, `reference.sqlite`/`manifest.json` publiés en release rolling `almanac-latest` par CI hebdo — vérifié vert 2026-08-08 ; **consommateurs app/Pi + notifs locales à faire**), puis événements calculés (conjonctions/oppositions/éclipses/showers), puis appulses. Discriminant oculaire/photo couplé **Macro 4** (seuil visuel via Setup tube) / **Macro 5** (seuil photo via Setup caméras). Notifs locales d'abord, FCM différé. Voir [ADR 2026-07-24](decisions.md) + [spec](../superpowers/specs/2026-07-24-oracle-ephemeres-design.md).
