...
| `GEOGRAPHIC_COORD` | Number RW | `LAT`, `LONG`, `ELEV` | push lat/lon depuis le site d'observation persisté (`observing_site`). AUX override `updateLocation()` à `celestronaux.cpp:2348` ; il met à jour le subsystem alignment + sync cordwrap pos. |
