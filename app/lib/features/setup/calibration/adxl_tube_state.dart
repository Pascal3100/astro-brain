...
/// `adxl_sigma_threshold=0.05`). Identiques à `adxl345_mount` car même
/// classe de capteur.
const adxlTubeMinSamples = 100;
const adxlTubeSigmaThreshold = 0.05;
...
class AdxlTubeState extends Equatable {
  const AdxlTubeState({
    this.status = AdxlTubeStatus.idle,
    this.progress,
    this.errorMessage,
    this.finalizedStatus,
  });

  final AdxlTubeStatus status;
  ...
  /// quand on a au moins [adxlTubeMinSamples] échantillons et un sigma
  /// strictement inférieur au seuil.
  bool get canFinalize =>
      status == AdxlTubeStatus.sampling &&
      progress != null &&
      progress!.samplesN >= adxlTubeMinSamples &&
      progress!.sigma < adxlTubeSigmaThreshold;

  AdxlTubeState copyWith({
    AdxlTubeStatus? status,
